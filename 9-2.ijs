...
NB. (pc;relbase;input;ouput;stack_count) day9_2 mem
day9_2=: 4 : 0
out=. ''
new_x=. x
new_y=. y
...
NB. stack_count=.0
NB. mem=. clean_numbers '1233,0,4,0,99'
NB. (pc;relbase;input;ouput;stack_count) Computer mem
...
case. 3 do.
  ...
  ((pc+2);rb;(}.in);out;>:sc) Computer new_mem
case. 4 do.
  mode=. {. (2&}. |. op) , 0
  param=. (pc + 1) r_fm_m y
  new_out=. (0 , mode ,. param) get_arg"(1 _) rb,y
  ((pc+2);rb;in;(out,new_out);>:sc) Computer y
fcase. 5 do.
case. 6 do.
  exe=. <`=@.(opc - 5)
  modes=. 0 1 { (2&}. |. op) , 2 # 0
